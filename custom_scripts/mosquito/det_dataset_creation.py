...
import pandas as pd
from datumaro import Bbox, DatasetItem
...
class MosquitoAlertDataset:
    ...
    def _make_dsitems(self):
        ...
        for index, row in self.df.iterrows():
            ...
            bbox = Bbox(x=x, y=y, w=w, h=h, label=label_idx, attributes=attributes)
    ...
    def strategy_0(self, n_folds=5, seed=0, export_path=None):
        ...
        for fold, (train_indices, val_indices) in enumerate(
            kf.split(range(len(self.dsitems)))
        ):
            ...
            if export_path is not None:
                dataset = Dataset.from_iterable(self.dsitems, categories=self.labels)
                dataset.export(
                    f"{export_path}/fold_{fold}", "coco", default_image_ext=".jpeg"
                )

    def export(self, dsitems, export_path, save_media=False):
        dataset = Dataset.from_iterable(dsitems, categories=self.labels)
        dataset.export(
            f"{export_path}", "coco", default_image_ext=".jpeg", save_media=save_media
        )
...
if __name__ == "__main__":
    dataset = MosquitoAlertDataset(data_root="/home/yuchunli/_DATASET/mosquito_data")
