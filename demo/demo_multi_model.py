...
def main():
    ...
    if isdir(args.inputs):
        dir = list_dir_or_file(args.inputs, list_dir=False, suffix=IMG_EXTENSIONS)
        for filename in dir:
    ...
    for i, (config, checkpoint) in enumerate(zip(args.config, args.checkpoints)):
    ...
    for i in range(len(results)):
        ...
        if not args.no_save_pred:
            out_json_path = args.out_dir + "/preds/" + img_name.split(".")[0] + ".json"
            mmengine.dump(
                {
        ...
        out_file = (
            osp.join(args.out_dir, "vis", img_name) if not args.no_save_vis else None
        )
