...
class DeticBBoxHead(Shared2FCBBoxHead):
    def __init__(
        ...
        **kwargs,
        ...
        self.init_cfg += [dict(type="Caffe2Xavier", override=dict(name="reg_fcs"))]
    ...
    def sigmoid_cross_entropy_loss(self, cls_score, labels):
        if cls_score.numel() == 0:
            return cls_score.new_zeros([1])[0]  # This is more robust than .sum() * 0.
        B = cls_score.shape[0]
    ...
    def image_label_losses(self, cls_score, sampling_results, image_labels):
        ...
        cls_score = cls_score.split(num_inst_per_image, dim=0)  # B x n x (C + 1)
        B = len(cls_score)
        loss = cls_score[0].new_zeros([1])[0]
        for score, labels, pred_instances in zip(
            cls_score, image_labels, sampling_results
        ):
            if score.shape[0] == 0:
    ...
    def refine_bboxes(
        self, bbox_results: dict, batch_img_metas: List[dict]
    ) -> InstanceList:
    ...
    def regress(self, priors: Tensor, bbox_pred: Tensor, img_meta: dict) -> Tensor:
