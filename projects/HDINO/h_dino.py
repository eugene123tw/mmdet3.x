...
class HDINO(DINO):
    def __init__(self, *args, bbox_head: OptConfigType = None, **kwargs) -> None:
        self.method = 0
    ...
    def pre_decoder(
        ...
        if self.method == 1:
            ...
            query = torch.gather(
                map_memory, 1, topk_indices.unsqueeze(-1).repeat(1, 1, self.embed_dims)
            )
        else:
        ...
        if self.training:
            ...
            num_denoising_queries = head_inputs_dict["dn_meta"]["num_denoising_queries"]
            num_query_one2one = num_denoising_queries + self.num_query_one2one
            # dn_mask[num_query_one2one:, :num_query_one2one] = True
            dn_mask[num_denoising_queries:num_query_one2one, num_query_one2one:] = True
