...
custom_imports = dict(imports=["projects.RF100-Benchmark"], allow_failed_imports=False)
...
model = dict(
    backbone=dict(norm_eval=False, norm_cfg=dict(requires_grad=True), frozen_stages=-1),
...
train_pipeline = [
    ...
    dict(
        type="RandomResize", scale=image_scale, ratio_range=(0.8, 1.2), keep_ratio=True
    ),
