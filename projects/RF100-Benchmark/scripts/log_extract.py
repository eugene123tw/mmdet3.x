...
def write_csv(datas, args):
    ...
    for dataset in sorted(os.listdir(datas)):
        ...
        num += 1
        with open(
            os.path.join(datas, dataset, "train/_annotations.coco.json"), "r"
        ) as f:
            image = json.load(f)
            num_train = len(image["images"])  # get number of train images
        with open(
            os.path.join(datas, dataset, "valid/_annotations.coco.json"), "r"
        ) as f:
            image = json.load(f)
        ...
        try:
            ...
            complete_flag = re.findall(
                r"Epoch\(val\) \[{}\]\[\d+/\d+\]".format(args.epoch), log
            )  # find log of args.epoch's validating process
            ...
            if not complete_flag:
                ...
                if match_all:
                    ...
                    # find best result
                    match_AP = re.findall(
                        r"\[{}\]\[\d+/\d+\]    coco/bbox_mAP: (-?\d+\.?\d*)  coco/bbox_mAP_50: (-?\d+\.?\d*)  coco/bbox_mAP_75: -?\d+\.?\d*  coco/bbox_mAP_s: (-?\d+\.?\d*)  coco/bbox_mAP_m: (-?\d+\.?\d*)  coco/bbox_mAP_l: (-?\d+\.?\d*)".format(  # noqa
                            best_epoch
                        ),
                        log,
                    )
...
def sum_excel(in_csv, out_xlsx):
    ...
    for i in range(1, 10):
        if i < 5:
            ...
            df_cate.append(
                format(
                    df_new.iloc[1:, i].astype(float).replace("", np.nan).mean(), ".4f"
                )
            )
...
def main():
    ...
    print(
        f"all {fail_num + none_exist_num} untrained datasets "
