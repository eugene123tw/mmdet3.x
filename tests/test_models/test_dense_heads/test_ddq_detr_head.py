...
class TestDDQDETRHead(TestCase):
    def test_ddq_detr_head_loss(self):
        ...
        references = list(
            torch.rand([1 + num_decoder_layers, batch_size, num_total_queries, 4])
        )
        all_layers_outputs_classes, all_layers_outputs_coords = bbox_head.forward(
            hidden_states, references
        )

        # logits
        enc_outputs_class = torch.randn([batch_size, num_distinct_queries, num_classes])
