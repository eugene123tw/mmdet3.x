...
class TestDDQDETR(TestCase):
    ...
    def test_ddq_detr_head_loss(self):
        ...
        configs = [get_detector_cfg("ddq/ddq-detr-4scale_r50_8xb2-12e_coco.py")]  # noqa E501
        ...
        for config in configs:
            ...
            for key, loss in empty_gt_losses.items():
                if "aux" in key:
                    self.assertEqual(
                        ...
                        (
                            r"Actual aux loss count for {} is {}," + r"it should be {}"
                ...
                if any(
                    zero_loss_keyword in key for zero_loss_keyword in zero_loss_keywords
                ):
                    self.assertEqual(
                        _loss,
                        0,
            ...
            random_images = torch.rand([len(batch_data_samples), 3, *batch_input_shape])
            ...
            model.eval()
            # test _forward
            model._forward(random_images, batch_data_samples=batch_data_samples_2)
            # test only predict
            model.predict(
                random_images, batch_data_samples=batch_data_samples_2, rescale=True
            )
